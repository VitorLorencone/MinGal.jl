...
function solve_linear_systems(A::Array, b::Array)
    ...
    lines::GAVector = [sum(Blade(1<<(j-1), A[j, i]) for j in 1:D) for i in 1:D]

    # The same thing but for the right side T = b1*e1 + b2*e2 +...+ bn*en
    T::GAType = sum(Blade(1<<(j-1), b[j]) for j in 1:D)
    ...
    ans = zeros(D)
    for i in 1:D
        substituted_vecs = [j == i ? T : A[j] for j in 1:D]
...
function solve_linear_systems_projective(A::Array, b::Array)
    ...
    lines = Multivector[]
    for i in 1:D
        mv = sum(Blade(1<<(j), A[i,j]) for j in 1:D) - b[i]*e0
        push!(lines, mv)
    end

    mv = dual(reduce(^, lines; init=Blade(1)))
    # mv = reduce(&, dual.(lines); init=eI) -> Alternative version, but worst
